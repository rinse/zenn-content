...
- 余米田の補題
  * カインド `Type -> Type` を持つ任意の型を関手に持ち上げる
- 自由モナド
  * 任意の関手をモナドに持ち上げる
...
`Free f` は `f` が表すアクションを順番に並べたデータである。
`Pure` は計算の結果を、`Bind` は一つのアクションとそれに続く計算を表す。
...
これがOperationalモナドを構成する一段目の変換である。

`liftCoyoneda` と `lowerCoyoneda` は、Haskellにおける余米田の補題に対応する。
圏論的には、共変関手 $F$ に対する余米田の補題は次の同型を与える。
...
1. `Coyoneda` によって任意の型 `f` を関手 `Coyoneda f` へ持ち上げる
...
```Haskell
interpretCommandFAsIO :: CommandF a -> IO a
interpretCoyoneda     :: (forall x. f x -> m x) -> Coyoneda f a -> m a
interpretFree         :: (forall x. f x -> m x) -> Free f a -> m a
interpretOperational  :: (forall x. f x -> m x) -> Operational f a -> m a
```
...
1. `Coyoneda` が、命令セット `f` を `Functor` 制約なしで関手へ持ち上げる
