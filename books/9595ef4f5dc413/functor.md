...
[![](https://storage.googleapis.com/zenn-user-upload/731904204c3c-20251029.png)](https://q.uiver.app/#q=)

ドメインの圏が双対圏 (*Opposite category*) であるような関手を、反変関手 (*Contravariant functor*) $F: \mathbf C^\mathrm{op} \to \mathbf D$ と呼ぶ。

反変関手は、対象 $X \in \mathrm{ob}(C)$ を対象 $F(X) \in \mathrm{ob}(D)$ に写すが、射については $\mathbf C$ の射 $f: X \to Y$ を $\mathbf D$ の射 $F(f): F(Y) \to F(X)$ に写す。

なお $\mathbf C$ にとって $F: \mathbf C^\mathrm{op} \to \mathbf D$ は反変関手であるが、$\mathbf C^\mathrm{op}$ にとって $F: \mathbf C^\mathrm{op} \to \mathbf D$ は共変関手である。
すなわち $F : \mathbf C^\mathrm{op} \to \mathbf D$ は $\mathbf C^\mathrm{op}$ の射 $f: X \to Y$ を $\mathbf D$ の射 $F(f): F(X) \to F(Y)$ に写す。

[![Contravariant](https://storage.googleapis.com/zenn-user-upload/c8fe05bc2ec7-20231203.png)](https://q.uiver.app/#q=)
...
また $F; G$ や $F \ggg G$ のように図式順で書く場合がある。図式順の演算子は、特に String diagram を書く場合に便利だ。
