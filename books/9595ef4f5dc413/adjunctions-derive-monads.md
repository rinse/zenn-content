...
このとき、3つ組$(G \circ F, \eta, G \ast \epsilon \ast F)$は圏$\mathbf{D}$上の[モナド](./monad.md)を構成する。（なお一般に圏$\mathbf{C}$上のモナドとは、関手$T: \mathbf{C} \to \mathbf{C}$と自然変換$\eta: 1_\mathbf{C} \to T$, $\mu: T^2 \to T$から成る3つ組$(T, \eta, \mu)$である）
