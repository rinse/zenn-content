...
$G$の$F$に沿った右カン拡張(*Right Kan extension of $G$ along $F$*)とは、関手$R: B \to C$と自然変換$\eta: R \circ F \Rightarrow G$からなる普遍射$(R, \eta)$である。
...
$G$の$F$に沿った左カン拡張(*Left Kan extension of $G$ along $F$*)とは、関手$L: B \to C$と自然変換$\epsilon: G \Rightarrow L \circ F$からなる普遍射$(L, \epsilon)$である。
...
右カン拡張の関手$R$と左カン拡張の関手$L$とを、カン拡張であることを強調して書く書き方がいくつかある。
